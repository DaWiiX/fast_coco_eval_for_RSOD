from .coco_eval_base import COCOeval as COCOevalBase
import copy
import itertools
import logging
import time
...
class COCOeval_faster(COCOevalBase):
    ...
    def evaluate(self):
        ...
        if p.iouType in set(["segm", "bbox", "boundary"]):
            ...
            raise ValueError(
                f"p.iouType must be segm, bbox, boundary or keypoints. Get {p.iouType}"
            )
    ...
    def stats_as_dict(self):
        if self.params.iouType in set(["segm", "bbox", "boundary"]):
            ...
            ValueError(
                f"iouType must be bbox, segm, boundary or keypoints. Get {self.params.iouType}"
            )
