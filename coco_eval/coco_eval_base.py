...
class COCOeval:
    ...
    def _prepare(self):
        ...
        if self.lvis_style:
            ...
            self.img_nel = {
                d["id"]: d.get("not_exhaustive_category_ids", []) for d in img_data
            }
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        raise DeprecationWarning(
            "COCOeval.evaluateImg deprecated! Use COCOeval_faster.evaluateImg instead."
        )

    def accumulate(self, p=None):
        raise DeprecationWarning(
            "COCOeval.accumulate deprecated! Use COCOeval_faster.accumulate instead."
        )

    def evaluate(self):
        raise DeprecationWarning(
            "COCOeval.evaluate deprecated! Use COCOeval_faster.evaluate instead."
        )
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            _count = 17 if self.lvis_style else 18
            ...
            stats[0] = self._summarize(
                1, areaRng='Small', maxDets=self.params.maxDets[0]
            )
            stats[1] = self._summarize(
                1, iouThr=0.50, areaRng='Small', maxDets=self.params.maxDets[0]
            )
            stats[2] = self._summarize(
                1, iouThr=0.75, areaRng='Small', maxDets=self.params.maxDets[0]
            )

            # SODA
            stats[3] = self._summarize(1, areaRng='AllRange', maxDets=self.params.maxDets[0])
            stats[4] = self._summarize(1, iouThr=0.50, areaRng='AllRange', maxDets=self.params.maxDets[0])
            stats[5] = self._summarize(1, iouThr=0.75, areaRng='AllRange', maxDets=self.params.maxDets[0])
            # stats[6] = self._summarize(0, areaRng='AllRange', maxDets=self.params.maxDets[0])
            ...
            # stats[7] = self._summarize(0, areaRng='Small', maxDets=self.params.maxDets[0])
            ...
            stats[6] = self._summarize(1, iouThr=0.50, areaRng='VeryTiny', maxDets=self.params.maxDets[0])
            stats[7] = self._summarize(1, iouThr=0.50, areaRng='Tiny', maxDets=self.params.maxDets[0])
            stats[8] = self._summarize(1, iouThr=0.50, areaRng='AiSmall', maxDets=self.params.maxDets[0])
            stats[9] = self._summarize(1, iouThr=0.50, areaRng='AiMedium', maxDets=self.params.maxDets[0])
            ...
            # SODA
            # stats[10] = self._summarize(0, areaRng='eS', maxDets=self.params.maxDets[2])
            # stats[11] = self._summarize(0, areaRng='rS', maxDets=self.params.maxDets[2])
            # stats[12] = self._summarize(0, areaRng='gS', maxDets=self.params.maxDets[2])
            # stats[13] = self._summarize(0, areaRng='Normal', maxDets=self.params.maxDets[2])

            # AiTOD
            stats[10] = self._summarize(1, areaRng='VeryTiny', maxDets=self.params.maxDets[0])
            stats[11] = self._summarize(1, areaRng='Tiny', maxDets=self.params.maxDets[0])
            stats[12] = self._summarize(1, areaRng='AiSmall', maxDets=self.params.maxDets[0])
            stats[13] = self._summarize(1, areaRng='AiMedium', maxDets=self.params.maxDets[0])


            # DOTD
            stats[14] = self._summarize(
                1, iouThr=0.50, areaRng='eS', maxDets=self.params.maxDets[0]
            )
            stats[15] = self._summarize(
                1, iouThr=0.50, areaRng='rS', maxDets=self.params.maxDets[0]
            )
            stats[16] = self._summarize(
                1, iouThr=0.50, areaRng='gS', maxDets=self.params.maxDets[0]
            )
            stats[17] = self._summarize(
                1, iouThr=0.50, areaRng='Normal', maxDets=self.params.maxDets[0]
            )
        ...
        def _summarizeKps_crowd():
            ...
            p = self.params
            iStr = " {:<18} {} @[ IoU={:<9} | type={:>6s} | maxDets={:>3d} ] = {:0.4f}"
        ...
        if iouType in set(["segm", "bbox", "boundary"]):
            ...
            ValueError(
                f"iouType must be bbox, segm, boundary or keypoints or keypoints_crowd. Get {iouType}"
            )
